...
# sudo env PATH=$PATH:/usr/bin /usr/lib/node\_modules/pm2/bin/pm2 startup systemd -u youruser --hp /home/youruser
...
#### Step 4 — Configure Nginx proxy\_pass for Node.js API
...
##### Why use 127.0.0.1 instead of localhost in proxy\_pass?
...
server {
    listen 80;
    server\_name app.yourdomain.com;
...
server {
    listen 80;
    server\_name admin.yourdomain.com;
    location / {
        proxy\_pass http://127.0.0.1:4000;
        proxy\_http\_version 1.1;
        proxy\_set\_header Host $host;
        proxy\_set\_header X-Real-IP $remote\_addr;
        proxy\_set\_header X-Forwarded-For $proxy\_add\_x\_forwarded\_for;
...
*   **502 Bad Gateway:** Nginx is running but cannot reach your Node.js app. Check that PM2 is running (`pm2 list`), verify the port number in proxy\_pass matches where your app actually listens, and check app logs with `pm2 logs`.
...
You now have the complete deployment stack: Node.js running under PM2, Nginx as a reverse proxy handling all public traffic via proxy\_pass, HTTPS enforced by Let's Encrypt, React static assets served directly by Nginx, and the ability to host multiple apps on a single server. This is the same stack used by the vast majority of Node.js deployments in production worldwide.
