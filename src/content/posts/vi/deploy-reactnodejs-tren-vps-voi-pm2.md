...
*   **max\_memory\_restart (250M):** Đây là tính năng quan trọng nhất. Nếu app của bạn bị rò rỉ bộ nhớ (Memory Leak) và chạm mức 250MB, PM2 sẽ tự động giết tiến trình đó và khởi động lại. Điều này giúp VPS không bao giờ bị treo cứng hoàn toàn.
*   **exec\_mode: "fork":** Khác với `cluster` (tạo ra nhiều bản sao để tận dụng đa nhân), `fork` chạy ứng dụng như một tiến trình đơn lẻ. Với VPS 1GB (thường chỉ có 1 vCPU), chế độ `fork` giúp giảm bớt gánh nặng quản lý cho hệ điều hành.
...
### 1\. Tại sao đã đặt max\_memory\_restart mà App vẫn bị OOM Killer?
