---
title: "Câu Hỏi Phỏng Vấn JWT Thường Gặp 2026: Đáp Án Chi Tiết"
...
description: "Tổng hợp 15+ câu hỏi phỏng vấn JWT thường gặp, đáp án chi tiết từ cấu trúc token, bảo mật, RS256 vs HS256 đến refresh token flow - tự tin vượt technical interview."
---

![Tổng hợp câu hỏi phỏng vấn JWT thường gặp cho developer 2026](https://cdn.khaizinam.io.vn/blog-folder/2026-05/jwt-interview-question.jpg)
...
- [1. JWT là gì? Câu hỏi nền tảng](#jwt-la-gi-cau-hoi-nen-tang-ban-phai-tra-loi-duoc)
- [2. Cấu trúc JWT - Header, Payload, Signature](#cau-truc-jwt--header-payload-signature)
- [3. JWT vs Session: So sánh trong phỏng vấn](#jwt-vs-session-so-sanh-trong-phong-van)
- [4. Câu hỏi bảo mật JWT nâng cao](#cau-hoi-bao-mat-jwt-nang-cao)
  - [4.1 RS256 vs HS256 - khi nào dùng gì?](#rs256-vs-hs256--khi-nao-dung-gi)
- [5. Access Token & Refresh Token - Flow thực tế](#access-token-refresh-token--flow-thuc-te)
- [6. 5 Sai lầm phổ biến khi trả lời về JWT](#5-sai-lam-pho-bien-khi-tra-loi-cau-hoi-ve-jwt-trong-phong-van)
- [7. FAQ - Câu hỏi thường gặp về JWT](#faq--cau-hoi-thuong-gap-ve-jwt-trong-phong-van)
- [Kết luận & Bước tiếp theo](#ket-luan-buoc-tiep-theo)
...
JWT (JSON Web Token) là một tiêu chuẩn mở ([RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519)) để truyền tải thông tin an toàn giữa các bên dưới dạng JSON object được ký số (signed). Điểm cốt lõi cần hiểu: JWT không phải là cơ chế mã hóa (encryption) - nó là cơ chế **xác minh tính toàn vẹn** (integrity verification). Ai cũng có thể đọc payload nếu có token, nhưng không ai giả mạo được nội dung mà không có secret key.
...
Câu hỏi hay bị hỏi thêm: **"Payload có bị lộ không?"** - Trả lời: Có. Base64URL là encode, không phải encrypt. Tuyệt đối không đặt password hay thông tin nhạy cảm trong payload. Nếu cần bảo mật payload, dùng JWE (JSON Web Encryption). Bạn có thể thử decode JWT ngay tại [jwt.io](https://jwt.io) để hiểu rõ hơn.

---

## JWT vs Session: So sánh trong phỏng vấn

### Khi nào dùng đúng công cụ
...
![Flow xác thực Access Token và Refresh Token trong JWT - best practice](https://cdn.khaizinam.io.vn/blog-folder/2026-05/access-token-refresh-token-flow.jpg)
...
Để hiểu sâu hơn về sự khác biệt giữa JWT và Session trong thực tế, bạn có thể đọc thêm [Session vs JWT Phỏng Vấn: Hướng Dẫn Đầy Đủ Để Không Bao Giờ Bị Hỏi Khó](/session-vs-jwt-phong-van).
...
### JWT lưu ở đâu là an toàn nhất?

Access token nên lưu trong memory (biến JavaScript) để tránh XSS; refresh token lưu trong httpOnly + Secure cookie để tránh cả XSS lẫn CSRF. Tránh dùng localStorage cho bất kỳ token nào trong môi trường production.

### JWT có thể dùng để xác thực WebSocket không?

Có. Truyền JWT qua query param khi kết nối WebSocket (ví dụ: `ws://api/ws?token=...`), server verify ngay tại handshake. Không dùng Authorization header vì browser WebSocket API không hỗ trợ custom header. Lưu ý đặt thời gian hết hạn ngắn cho token dùng trong WebSocket handshake.
...
**Tác giả: Nguyễn Hữu Khải** - Backend Developer với kinh nghiệm xây dựng hệ thống xác thực và API cho các sản phẩm SaaS. Viết tại [khaizinam.io.vn](https://khaizinam.io.vn).

*Cập nhật lần cuối: 21/04/2026*

> ## Đọc tiếp trong Series Phỏng Vấn Backend
>
> Nếu bạn muốn hiểu sâu hơn về authentication và authorization:
>
> - [Session vs JWT: Toàn Bộ Lý Thuyết](/session-vs-jwt-toan-bo-ly-thuyet) - giải thích đầy đủ cơ chế hoạt động, ưu nhược điểm và kiến trúc thực tế.
> - [Session vs JWT Phỏng Vấn](/session-vs-jwt-phong-van) - tổng hợp câu hỏi interview phổ biến kèm câu trả lời ngắn gọn.
> - [Session vs JWT: Developer Nên Chọn Cái Nào?](/session-vs-jwt-developer-nen-chon-cai-nao) - phân tích khi nào nên dùng Session, khi nào nên dùng JWT trong production.
> - [JWT Security Best Practices](/jwt-security-best-practices) - checklist bảo mật JWT quan trọng mà nhiều developer thường bỏ sót.
