...
- [7. Những sai lầm tôi thấy người dùng hay mắc khi migrate](#7-nhung-sai-lam-toi-thay-nguoi-dung-hay-mac-khi-migrate)
- [8. FAQ](#8-faq)
...
##### 5.2. Ưu điểm mạnh nhất của Astro: PageSpeed luôn "hoàn hảo"

![Ưu điểm mạnh nhất của Astro PageSpeed luôn hoàn hảo](https://cdn.khaizinam.io.vn/blog-folder/2026-05/Screenshot%202026-05-25%20090018.png)

Chi phí để optimize technical SEO cho Astro gần như bằng 0. Khác với WordPress - nơi bạn phải trả tiền mua các plugin tối ưu và tốn nhiều công sức để cố gắng đạt điểm xanh - Astro bản chất đã cực kỳ nhẹ, trang tải rất nhanh, và điểm PageSpeed hầu như luôn đạt mức 95-100 ở cả Mobile lẫn Desktop mà không cần phải tinh chỉnh thêm.
...
#### 7. Những sai lầm tôi thấy người dùng hay mắc khi migrate
...
Bạn nên sử dụng kết hợp GitHub để lưu trữ mã nguồn và bài viết. Nếu chỉ sử dụng phương pháp truyền thống là tải trực tiếp từng file lên Cloudflare, khi hệ thống Cloudflare gặp sự cố hoặc bạn lỡ tay xoá, bạn có thể mất toàn bộ dữ liệu. GitHub đóng vai trò là một nơi lưu trữ dự phòng (backup) cực kỳ an toàn. Hãy tạo thói quen commit đều đặn sau mỗi lần viết hoặc sửa bài. Đừng lo về việc deploy, vì Cloudflare Pages sẽ tự động trigger quá trình build mỗi khi có commit mới trên GitHub.

##### 7.6. Quên tạo custom domain cho tiền tố www

Nhiều người chỉ cấu hình tên miền chính (ví dụ: `domain.com`) mà quên thêm `www.domain.com` vào phần Custom Domains của Cloudflare Pages. Điều này dẫn đến việc người dùng truy cập qua `www` sẽ bị báo lỗi hoặc timeout, gây ảnh hưởng trực tiếp đến trải nghiệm và làm giảm điểm SEO đáng kể.

##### 7.7. Quên cấu hình Cache TTL cho R2

Nếu bạn sử dụng R2 để lưu trữ ảnh, hãy thiết lập Cache TTL của domain thành 1 năm. Rất nhiều người để mặc định là 4 giờ, khiến trình duyệt liên tục tải lại ảnh, làm giảm điểm PageSpeed và khiến hệ thống cache file không phát huy được hiệu quả.
